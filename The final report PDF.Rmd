...
---
  ...
  In this assignment, I have chosen to use linear regression, logistic regression, and regression tree models to analyze and compare models in the training set data to predict the survival of passengers in the test dataset. The data consists of 11 variables, but only 8 are applicable for the comparative model. This dataset is not very high-dimensional, and all the key predictors are already well defined. For models that can inherently handle both numerical and categorical data and consider interaction terms (such as linear regression and logistic regression), there may not be a need for extensive dimensional reduction to perform well. Therefore, I did not choose principal component analysis (PCA) and Lasso/Double Lasso regression. Furthermore, I chose the regression tree model to handle complex, non-linear interactions between variables, which both linear and logistic regression cannot handle.


  Even the 


  Thr proference base on the Kaggle scores demostrict both linear and logisgtic regression model got the same score of 0.77033 by using a set of variables including Pclass, Age, SibSp, Embarked, and Sex_female. however the
  
  

According to my analysis, performance metrics from Kaggle indicate a identitcal match between linear and logistic regression using variable set two (Pclass, Age, SibSp, Embarked, Sex_female) at 0.77033, with the regression tree model slightly outperforming them at 0.777051, enhancing accuracy by 0.00718. 
...
In this step, I divided the training data set into two parts: 90% of the data will be used for training, and the remaining 10% will serve as the test set. This division will allow us to calculate the Mean Squared Error, Root Mean Squared Error, and Mean Absolute Error to evaluate the performance of two sets of linear regression and two sets of logistic regression models. MSE and RMSE impose larger penalties for higher prediction errors, such that the cost of large errors is more significant. As for MAE, focusing more on the general performance of the model, it provides an average error without specifically penalizing large errors.


And then I combine the Kaggle prediction scale together with the table.
...
The logistic regression model with variable set two has the lowest MSE, RMSE, and MAE compared to the other three models. However, the logistic regression model and the linear regression model with the same variable set have identical Kaggle prediction scales. A possible explanation is that I set the output to 1 when the model's predicted output is higher than 0.5, and to 0 when the model's predicted output is lower than 0.5. As a result, even though the logistic regression model performs better than the linear regression model in terms of MSE, RMSE, and MAE, the improvements are not significant enough to increase the probability of survival to change the output.
